...
class Concat(nn.Module):
    ...
    def forward(self, x):
        # Ensure input is a non-empty list or tuple of tensors
        if not isinstance(x, (list, tuple)) or len(x) == 0:
            raise ValueError("Input to Concat must be a non-empty list or tuple of tensors")

        # Ensure all elements are tensors
        for tensor in x:
            if not isinstance(tensor, torch.Tensor):
                raise TypeError(f"All elements in input must be tensors, but got {type(tensor)}")

        # Check if all tensors have the same shape except in the concatenation dimension
        base_shape = x[0].shape
        for tensor in x:
            if tensor.shape[:self.d] != base_shape[:self.d] or tensor.shape[self.d+1:] != base_shape[self.d+1:]:
                raise ValueError(f"Tensor shapes must match in all dimensions except {self.d}. "
                                 f"Expected shape {base_shape}, but got {tensor.shape}")

        # Perform concatenation along the specified dimension
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.bn1 = nn.BatchNorm2d(out_channels) if stride == 1 else nn.Identity()
        self.relu = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channels) if stride == 1 else nn.Identity()

        self.shortcut = nn.Sequential()
        if stride != 1 or in_channels != out_channels:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(out_channels) if stride == 1 else nn.Identity()
            )
